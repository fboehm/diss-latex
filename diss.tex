...
These developments, when coupled with experiments to quantify the gut microbiome, have the potential to uncover new microbiome-host interactions. 
...
Previous research demonstrated that hosts and gut microbiome communicate with each other via small molecule metabolites, including bile acids. 
...
While some bile acids are lost in the feces, much of the secreted bile acids is reabsorbed in the distal gut. 
...
Processing of fecal samples to obtain microbial taxa counts 
...
We subjected plasma samples to multiple experimental and computational steps to obtain traits for QTL mapping. 
...
We extracted DNA from tail clippings and subjected it to SNP genotyping with the GigaMUGA microarray \citep{morgan2015mouse}. 
...
We then had the inputs needed for our pleiotropy test: founder allele probabilities for each mouse at every marker and two traits that map to a single genomic region. 
We performed a test of pleiotropy vs. separate QTL for our two traits, 
\emph{Turicibacter} abundance and plasma cholic acid levels, with the methods of Chapter 2. 
...
bootstrap p-value as the proportion of the 1000 tests, 
denoted by $\Lambda_i$, with $i$ ranging from 1 to 1000, 
that were at least as extreme as the true test statistic $\Lambda$ (Equation \ref{eq:bootp}). This calculation of a bootstrap p-value is the last step in our hypothesis test for pleiotropy vs. separate QTL.
