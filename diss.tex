...
\label{eq:pooled-var}
\end{equation}
The residual sum of squares ``RSS$_1$'' is an expression that
...
These hypotheses (Equation~\ref{eq:qtl-hypotheses}) are equivalent to stating that, under $H_0$, 
$\beta = 0$ while under $H_1$, $\beta$ can be any number. That is, under the null hypothesis, those with genotype AB have the same mean value as those with genotype AA, while under the alternative hypothesis, the means for the two genotype classes may differ. 
Those with backgrounds in statistics may recognize that the likelihood ratio test for presence of a QTL in a two-parent backcross is equivalent to a t-test. 
