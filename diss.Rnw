...
<<do, fig.cap = "Diversity outbred mouse genomes are a highly heterozygous mixture of eight founder genomes.">>=
##############################
# HS
##############################
...
founders' DNA. Figure~\ref{fig:do} represents unique Diversity Outbred mice as a pair of 
autosomes (vertical bars). DNA from each founder line is color-coded. Horizontal rows 
correspond to unique generations, with generation number increasing from top to bottom. 
With each new generation, the Diversity Outbred mouse genomes are shuffled
in smaller and smaller chunks.
The smaller DNA chunk sizes in later generations correspond to greater QTL mapping 
resolution.
