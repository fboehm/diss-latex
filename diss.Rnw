...
backcross, the ``B allele count'' variable, also denotes $x_i$, where $i$ indexes the subjects, takes values of 0 or 1. In an intercross
...
y_i &=& \mu_{AA} + x_i\beta + \epsilon_i
\label{eq:uni-2-parent}
\end{eqnarray}

\begin{equation}
\epsilon_i \sim N(0, \sigma^2), \hspace{1cm}\epsilon_i\text{ independent}
\label{eq:random-error-dist}
\end{equation}

Equation~\ref{eq:random-error-dist} specifies the random error distribution. 
Equation~\ref{eq:random-error-dist} also introduces 
notation that requires explanation. 
...
and variance, completely specify a normal distribution. Equation~\ref{eq:normal-pdf} gives a mathematical definition of the normal 
probability density. 
...
parameters, ``mean for AA genotype'' ($\mu_{AA}$), ``B allele effect'' ($\beta$), and the variance ($\sigma^2$) of the random errors. In this 
...
\begin{equation}
\hat\mu_{g} = \frac{\sum_{i = 1}^ny_i\mathbbm{1}{g_i = g}}{n_{g}}
\label{eq:mu-g-i-hat}
\end{equation}

In Equation~\ref{eq:mu-g-i-hat}, $g$ is any of AA, AB, or BB. $\mathbbm{1}{g_i = g}$ 
takes values 0 and 1; if $g_i = g$, then $\mathbbm{1}{g_i = g} = 1$; otherwise, it's zero.
...
I continue this discussion in Section~\ref{sec:materials:methods}.
