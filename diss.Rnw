...
I define the fitted trait value, $\hat y_i$, for the $i^{th}$ subject with Equation~\ref{eq:fitted-values}. 

\begin{equation}
\hat y_i = \text{(fitted trait value)} = x_i\hat b
\label{eq:fitted-values}
\end{equation}

where $x$ is the vector of founder allele dosages for subject $i$ and $\hat b$ is the ordinary least squares estimate of $b$. 

To obtain a formula for $\hat b$, I substitute Equation~\ref{eq:fitted-values} into Equation~\ref{eq:rss} to express the residual sum of squares in Equation~\ref{eq:rss-2}. Note that I remove the ``hat'' from $b$ in Equation~\ref{eq:rss-2} to emphasize that $b$ is a variable. 

\begin{equation}
\text{residual sum of squares}(b) = \sum_i(y_i - x_ib)^2
\label{eq:rss-2}
\end{equation}

The ``best fit'' line is that which minimizes the residual sum of squares. 
Thus, we need to solve for $\hat b$, \emph{i.e.}, the value of $b$
that minimizes the residual sum of squares.
We first recognize that we've written the residual sum of squares as
a function of $b$.
We draw on results from calculus, by which we know that we can
differentiate (with respect to $b$) the continuous and differentiable
residual sum of squares function.
Setting the derivative equal to zero, we then solve for $\hat b$. 


\begin{eqnarray}
0 &=& \frac{\partial}{\partial b}\left(\text{residual sum of squares} \right) \\
&=& \frac{\partial}{\partial b}\left(\sum_i(y_i - x_ib)^2\right)\\
&=& -2\sum_i x_i(y_i - x_ib)\\
&\implies& \\
\sum_i x_iy_i &=& b\sum_i x_i^2\\
&\implies& \\
b &=& \frac{\sum_i x_iy_i}{\sum_i x_i^2} = \hat b
\end{eqnarray}
