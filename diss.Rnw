...
The index range for the terms are given above and below the $\prod$ symbol. Here, $n$ is our sample size.
In Equation~\ref{eq:lik-bc}, our three parameters are ``AA mean trait value'' 
...
Those readers with backgrounds in statistics may recognize that the likelihood ratio
test for presence of a QTL in a two-parent backcross is equivalent to a t-test. 

After calculating the likelihood ratio test statistic at every marker,
one needs to obtain p-values for every test. Because a statistician performs many
