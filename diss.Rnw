...
genomic regions that affect complex traits, such as systolic blood pressure and body weight \citep{broman2009guide,jansen2007quantitative}.
...
A region with strong evidence of association, then, defines a QTL. 
...
Identification of the causal gene from among those genes near the QTL is
...
statistical tools to fully examine the large volumes of data \citep{keller2018genetic}. 
A test of pleiotropy
vs. separate QTL is one multivariate statistical tool that will inform
complex trait genetics.
In this thesis, I develop a test of pleiotropy vs. separate QTL in multiparental
populations. I study its statistical properties and demonstrate its usefulness
...
AA founder line mean. It's also valuable to consider the range in the observations in each 
...
calculation of the pooled variance estimate, is an expression that
...
H_0: \text{No QTL is present at a specified marker}\label{eq:qtl-hypotheses}
\end{eqnarray}

These hypotheses (Equation~\ref{eq:qtl-hypotheses}) are equivalent
to stating that, under $H_0$, 
$\beta = 0$ while under $H_1$, $\beta$ can be nonzero.
In other words, under the null hypothesis, those with genotype AB 
have the same
mean trait value as those with genotype AA, while under the alternative hypothesis,
the mean trait values for the two genotype classes may differ. 
...
, $X$ is a $2n$ by $4$ block-diagonal matrix containing genotype data for two positions, 
...
\citet{jiang1995multiple} use a statistical ``mixture model'' to relate genotypes to phenotypes. 
...
expectation-conditional maximization algorithm, a variant of the expectation maximization
...
pairs of markers in the genomic region of interest. The restriction of that collection
...
(Figure~\ref{fig:encircle}). Each point in the grid corresponds to an ordered pair of
...
Collaborative Cross lines \citep{churchill2004collaborative}. 
...
in the first generation (Figure~\ref{fig:ri8}).
