...
genomic regions (termed ``quantitative trait loci (QTL)'') that affect complex traits, such as systolic blood pressure and body weight \citep{sax1923association,lander1989mapping,broman2009guide,jansen2007quantitative}.
...
<<bc-pheno, fig.cap="Simulated hypothetical phenotype data for two inbred lines, F$_1$ progeny, and a backcross population (Figure 1.4 from \\cite{broman2009guide})", fig.lp="fig:", fig.height = 3, fig.width = 6, out.width='\\textwidth'>>=
# http://www.rqtl.org/book/RqtlBook_Code/fig01.R -- figure 1.4
set.seed(4131972)
data(map10)
...
stripchart(x, method="jitter", pch=1, xlab="Phenotype", cex.lab = 1.5)
abline(h=1:4, lty=2)
me <- sapply(x, mean)
...
<<backcross, fig.cap = "Breeding scheme for a backcross. Each pair of autosomes represents a single subject. (From \\citet{broman2009guide})", fig.lp="fig:", out.height = "5in">>=
...
<<intercross, fig.cap = "Breeding scheme for an intercross. Each pair of autosomes represents a single subject. (From \\citet{broman2009guide})", fig.lp="fig:", out.height="5in">>=
par(mar=rep(0.1,4), bty="n")
plot(0,0,xlim=c(-15,864),ylim=c(0,480),xaxt="n",yaxt="n",xlab="",ylab="",type="n")
xd <- 2
...
\item a genetic map that contains genomic positions for all molecular markers (Figure~\ref{fig:map})
\end{enumerate}
...
<<map, fig.width = 6, fig.height = 3, out.width = "6in", fig.cap="Genetic map for markers from \\citet{sugiyama2001concordance}. Figure from \\citet{broman2009guide}.">>=
# http://www.rqtl.org/book/RqtlBook_Code/fig01.R
######################################################################
# Figure 1.6
#
...
genome (Figure~\ref{fig:map}). Some chromosome regions, including parts of 
...
After obtaining likelihoods, likelihood ratio test statistics are calculated
...
In systems genetics studies, the analyst typically visualizes likelihood ratio test
...
Below, we discuss methods for genotype inference at positions between markers. 
%\begin{figure}
...
<<genoprob1, fig.cap="Inferring genotypes at an intermarker position (from \\citet{broman2009guide})", out.height="2in", fig.height = 2, fig.width = 6.5, eval = TRUE>>=
bw <- FALSE
source("colors.R")
...
\begin{equation}
\text{(random error)} \sim N(0, \sigma^2), \hspace{1cm}\text{      (random errors) independent}
\label{eq:random-error-dist}
\end{equation}

Equation~\ref{eq:random-error-dist} introduces notation that requires explanation. 
The ``$\sim$'' symbol designates a probability distribution. The left-hand
side ``is distributed'' the right-hand side. In this case, I describe the distribution of the 
random errors. Capital ``N'', in a distribution, abbreviates ``Normal''. The first number after 
the ``N'' is the distribution mean, while the second is the variance. These two numbers, mean 
and variance, completely specify a normal distribution.




The linear regression equation (Equation~\ref{eq:uni-2-parent}) has three statistical 
parameters, ``mean for AA genotype'', ``B allele effect'', and the variance of the random errors. Notice that 
the variance of the random errors is not explicitly written in 
Equation~\ref{eq:uni-2-parent}, while ``mean for AA genotype'' and ``B allele effect'' are obviously part 
...
estimation are in Appendix~\ref{sec:stat-primer}.

To say that a statistical model is correctly specified means, here, that the family, or functional form, of the model is correct. If I say that the data follow a normal distribution, then they do, in fact, follow a normal distribution. I generally will not know the true parameter values, but, in the case of the random errors in Equation~\ref{eq:random-error-dist}, the model is correctly specified if the random errors really do follow a normal distribution with some positive variance $\sigma^2$.
...
Equation~\ref{eq:rss1} by replacing ``B allele count'' with $x$. In 
Equation~\ref{eq:rss1}, I replace ``mean for AA genotype'' with $\mu_{AA}$. The Greek 
letter $\mu$ is widely used in statistics to designate a mean, while the subscript ``AA'' 
tells us the genotype class.
...
hypothesis H$_1$. The likelihood of the model parameters is equal, by definition, to the joint 
...
I also need to calculate the maximum likelihood under the null model constraints, \emph{i.e.}, under the assumption that there is no QTL at the marker, in which case the genotype classes have the same mean trait value. 
...
\subsection{Interval mapping}\label{sec:im}

I highlight elements from the exposition in \citet{broman2009guide}. More details are available in its Chapter 4. \citet{lander1989mapping} describe interval mapping for QTL studies. While marker 
regression considers only marker positions as candidate QTL, in interval mapping 
one probabilistically infers genotypes for all genomic positions on a grid 
...
markers 1 and 2 (Table~\ref{tab:genoprob}).
...
(Equation~\ref{eq:joint-conditional-probs}). Sequential application of this relationship yields 
the probabilities in Table~\ref{tab:genoprob}. A more general solution for these calculations can be implemented with algorithms for hidden Markov models (Appendix~\ref{sec:stat-primer}) \citep{broman2009guide, broman2006use}.
...
In interval mapping, trait values are modeled statistically as having a distribution 
that is a mixture of normal distributions, where each component in the mixture correspon
ds to a genotype class. I assume a single variance common to all components (at a single 
putative QTL). In a backcross, there are thus two normal distribution components for 
each grid point, \emph{i.e.}, each putative QTL. Fitting such a model by maximum 
likelihood requires an iterative procedure like the expectation-maximization algorithm 
...
\subsection{Haley-Knott regression}\label{sec:hk}
...
<<pleiotropy-2traits, fig.height = 3, fig.width = 6.5, out.width = "\\textwidth", fig.cap = "One pleiotropic QTL. Figure after code from https://github.com/kbroman/QTLFigs repository.">>=
...
x <- c(25, 75)
y <- seq(10, 90, len=5)
text(x[1], y[3], expression(Q), cex = 2)

text(x[2], y[2], expression(Y[2]), cex = 2)
#text(x[2], y[3], expression(Y[2]))
text(x[2], y[4], expression(Y[1]), cex = 2)
...
<<separate-2traits, fig.height = 3, fig.width = 6.5, out.width = "\\textwidth", fig.cap = "Two separate QTL. Figure after code from https://github.com/kbroman/QTLFigs repository.">>=
...
x <- c(25, 75)
y <- seq(10, 90, len=5)
text(x[1], y[2], expression(Q[2]), cex = 2)
text(x[1], y[4], expression(Q[1]), cex = 2)
...
They observe that, just as there are three genotype classes for a single trait in an intercross, there are nine possible ordered pairs of genotypes for two
...
\citep{dempster1977maximum,meng1993maximum}. In this sense, their strategy involves a 
multivariate version of interval mapping. 
...
I illustrate these ideas in a drawing of a two-dimensional grid
...
The designers of the Collaborative Cross used a ``multi-funnel'' mating scheme to 
generate
...
founders' DNA \todo{add figure for one mouse's DNA?? like figure 5A in Svenson et al. 2012}
...
opportunities for recombinations. \todo[inline]{need to explain this better. How does Andrew explain it in his thesis?}
...
Chapter 4 consists of a user guide to the R software package \texttt{qtl2pleio}, which is
...
%Deoxyribonucleic acid (DNA) arranges itself in a double helix. A single DNA polymer features a deoxyribose backbone with one of four nucleosides appended to each deoxyribose unit. The four nucleosides are: adenine, thymine, cytosine, and guanine. 
