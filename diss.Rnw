...
More details are available in its Chapter 4. \citet{lander1989mapping} describe interval 
...
directions. For example, ``Q'' causes ``M''. ``M'' designates the mediator (local transcript), while ``Q'' designates the QTL (or DNA variant). ``Y'' denotes the nonlocal transcript 
