...
bgcolor <- "white"
c1 <- color[6]
par(fg=c1,col=c1,col.axis=c1,col.lab=c1,
      bg=bgcolor)
...
Equation~\ref{eq:random-error-dist} specifies the random error distribution. It also introduces notation that requires explanation. 
The ``$\sim$'' symbol designates a probability distribution. The left-hand
side ``is distributed as'' the right-hand side. In this case, I describe the distribution of the 
...
seek the parameter values that are most compatible with the data. To do this, I use the approach 
...
models (Appendix~\ref{sec:stat-primer}) \citep{broman2009guide, broman2006use}.
...
par(fg=c1, col=c1,col.axis=c1,col.lab=c1,col.main=c1,
    bg=bgcolor, 
    mar=rep(0.1, 4), bty="n")
par(mfrow = c(1,2))
...
<<svenson, fig.cap = "Color-coded genome of a G4 Diversity Outbred mouse (from \\citet{svenson2012high})", out.width = "6.5in", eval = FALSE>>=
include_graphics("figs/svenson2012.pdf")
@


<<do, fig.cap = "Diversity outbred mouse genomes are a highly heterozygous mixture of eight founder genomes. (R code to create figure from https://github.com/kbroman/QTLFigs repository.)", eval = TRUE>>=
##############################
# HS
##############################
bgcolor <- "white"
color_col <- "black"
par(mar=rep(0.1,4),las=1,fg=color_col,col=color_col,col.axis=color_col,
    col.lab=color_col,
    bg=bgcolor)
...
for(i in 1:n.lines)
  gen[[i]] <- simcross::create_parent(100, sample(1:8, 2))
...
for(i in 1:(n.lines/2)) {
  gen[[i*2-1]] <- simcross::cross(old[[i*2-1]], old[[i*2]])
  gen[[i*2]] <- simcross::cross(old[[i*2-1]], old[[i*2]])
}
...
founders' DNA. Figure~\ref{fig:do} presents color-coded chromosomes for a collection of subjects from a multiparental population like the Diversity Outbred mice. DNA from each founder 
line is color-coded. Each pair of vertical bars corresponds to one subject. Each row corresponds to a single generation. Generation number increases from top to bottom. 
...
nuclear autosomal markers and pseudomarkers. The outputted genotypes have 36 states because each position has eight homozygote states (one for each founder line) and $\binom{8}{2}$ heterozygote states. Pseudomarkers are 
