...
parameters, ``mean for AA genotype'', ``B allele effect'', and the variance of the random errors. In this 
statistical framework, I treat parameters as fixed, but unknown, numbers. I then 
seek the parameter values that are most compatible with the data. I use the approach 
called ``maximum likelihood estimation''. 



The maximum likelihood estimators for two parameters, ``B allele effect'' and 
...
Equation~\ref{eq:rss1}, I replace ``mean for AA genotype'' with $\mu_{AA}$. 
...
On the right side of Figure~\ref{fig:pleiotropy-2traits} two distinct QTL (Q$_1$ and Q$_2$) affect one trait each. The pink arrows indicate that Q$_1$ and Q$_2$ are on the same chromosome and are correlated by linkage disequilibrium.
...
\section{Designs for multiparental populations and Diversity Outbred mice}\label{sec:multiparent-designs}

% overview of MPP mapping populations. What are the essential ideas?
...
for study of a wide variety of quantitative traits. 
The magnitude of such an undertaking required a collaborative, research community-supported
...
#       pointsize=14)
color_col <- "black"
arrow_color <- "black" # arrows() uses - by default - par("fg")
par(mar=rep(0.1,4),las=1,fg=arrow_color,col=color_col,col.axis=color_col,col.lab=color_col,
    bg=bgcolor,bty="n")
...
Thus, Diversity Outbred mice from later generations have greater
...
nuclear autosomal markers and pseudomarkers. \todo[inline]{Explain why genotypes involve 36 states.}Pseudomarkers are 
